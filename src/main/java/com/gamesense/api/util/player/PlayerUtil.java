...
public class PlayerUtil {
    ...
    public static void centerPlayer(Vec3d centeredBlock, double diffX, double diffZ) {
        ...
        if (xDeviation <= 0.1 && zDeviation <= 0.1) {
            ...
            mc.player.connection.sendPacket(new CPacketPlayer.Position(newX + diffX, mc.player.posY, newZ + diffZ, true));
            mc.player.setPosition(newX + diffX, mc.player.posY, newZ + diffZ);
        }
    }
}